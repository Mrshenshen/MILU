...
class QADataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if "descriptions" in self.annotations[index].keys():
            descriptions = self.annotations[index]["descriptions"]
        else:
            descriptions = None

        # if "interactive" in self.annotations[index].keys():
        #     interactive = self.annotations[index]["interactive"]
        # else:
        #     interactive = None

        if "answer_list" in self.annotations[index].keys():
            answer_list = pad_sequence([torch.tensor(ts) for ts in self.annotations[index]["answer_list"]], batch_first=True)
        else:
            answer_list = None
...
class JointDataset(ConcatDataset):
    ...
    def collate_fn(self, batch):
        ...
        # print(type(batch[0]['descriptions']))
        descriptions = [b['descriptions'] for b in batch]
        # interactive = [b['interactive'] for b in batch]
        answer_list = [b['answer_list'] for b in batch]
        ...
        if descriptions != None:
            descriptions_tok = self.tokenizer(descriptions, padding=True, return_tensors='pt', add_special_tokens=False).input_ids
        else:
            descriptions_tok = None
        if descriptions_tok != None:
            masked_description = descriptions_tok
            mask_rate = 0.2
            tot_mask_num = []
            tot_mask_idx = []
            # print("asdasdasd", descriptions_tok.shape)
            for num, one_token in enumerate(descriptions_tok):
                mask_num = int(one_token.shape[-1] * mask_rate)
                tot_mask_num.append(mask_num)
                mask_idx = torch.randperm(one_token.shape[-1]-1)[:mask_num]
                tot_mask_idx.append(mask_idx)

                for idx in mask_idx:
                    masked_description[num][idx] = self.tokenizer.convert_tokens_to_ids('<unk>')
            tot_mask_idx = torch.cat(tot_mask_idx, dim=0)
        else:
            tot_mask_idx = None
            tot_mask_num = None
            masked_description = None

        # if None in interactive:
        #     interactive = None
        # if interactive != None:
        #     interactive_token = []
        #     interactive_mask = []
        #     # print(interactive)
        #     temp = [[interactive[j][i] for j in range(len(interactive))] for i in range(len(interactive[0]))]
        #     for one_question in temp:
        #         one_question_token = self.tokenizer(one_question, padding=True, return_tensors='pt', add_special_tokens=False)
        #         interactive_mask.append(one_question_token.attention_mask.bool())
        #         interactive_token.append(one_question_token.input_ids)
        ...
        result = {
            ...
            'descriptions_token': descriptions_tok,
            'mask_idx': tot_mask_idx,
            'mask_num': tot_mask_num,
            'masked_description': masked_description,
            # 'interactive_token': interactive_token,
            # 'interactive_mask': interactive_mask,
            'answer_list': answer_list
        }
