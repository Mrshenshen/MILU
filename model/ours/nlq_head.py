...
class NLQHead(nn.Module):
    ...
    def forward(self, feat, mask, training=True, gt_segments=None, gt_labels=None, v_lens=None, feat_cd = None):
        """
        feat: (B, D, T)
        mask: (B, 1, T)
        """
        ...
        if feat_cd is not None:
            # TODO: use cd logits and logits to predict new logits
            fpn_feats_cd, fpn_masks_cd = self.neck([feat_cd], masks)
            points_cd = self.point_generator(fpn_feats_cd)
            out_cls_logits_cd = [x.permute(0, 2, 1) for x in self.cls_head(fpn_feats_cd, fpn_masks_cd)]  # (B, T, #cls+1)
            out_offsets_cd = [x.permute(0, 2, 1) for x in self.reg_head(fpn_feats_cd, fpn_masks_cd)]  # (B, T, #cls*2)
            # fpn_masks_cd = [x.squeeze(1) for x in fpn_masks]  # (B, T)
        ...
        if training:
            ...
            if feat_cd is None:
                ...
                # TODO: Apply vcd equation
                alpha, beta = (0.1, 1)
                # print("abnormal feat")
                # print(type(alpha), out_cls_logits.dtype)
                logits_vcd = [F.softmax((1+alpha)*one_out_cls_logits-alpha*one_out_cls_logits_cd) \
                                        for one_out_cls_logits,one_out_cls_logits_cd in zip(out_cls_logits, out_cls_logits_cd)]

                offsets_vcd = [(1+alpha) * one_out_offsets - alpha * one_out_offsets_cd \
                               for one_out_offsets, one_out_offsets_cd in zip(out_offsets, out_offsets_cd)]
                
                points_vcd = [(1+alpha) * one_points - alpha * one_points_cd for one_points, one_points_cd in zip(points, points_cd)]
                results = self.inference(points_vcd, fpn_masks, logits_vcd, offsets_vcd, 1, v_lens)
                # results = self.inference(points, fpn_masks, logits_vcd, out_offsets, 1, v_lens)
            return results
    ...
    # def inference_cd(self, points, fpn_masks, out_cls_logits, out_offsets, num_classes, v_lens, points_cd, out_cls_logits_cd, out_offsets_cd):
    #     results = []
